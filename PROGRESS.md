...
- **Next (priority order):** **Complex task handling / project execution** on top of the world model — Projects are the durable goal/context container; Tasks are the execution grain. A request like “create three named avatars” should become one project plus three avatar-creation tasks routed by capability/stewardship, with blockers, approval, completion evidence, and telemetry when a tool is used incorrectly ([`docs/WORLD_MODEL_AND_PREPROCESSOR.md`](docs/WORLD_MODEL_AND_PREPROCESSOR.md), [`docs/IMPLEMENTATION_ROADMAP.md`](docs/IMPLEMENTATION_ROADMAP.md)). In parallel, improve **search-result-driven avatar creation** so named-avatar form fields are filled from better field-specific searches, follow-up queries, source evidence, and confidence. Then continue SPEC `data/metadata/` / durable backing, Active Task / Focus Watcher, and **Conversation archive** segments/chapters. **Proactive:** timer/cue; **sequential release batch** polish **deprioritized** (MVP acceptable).
...
- **Priority realignment (2026-04-26):** Next work shifts toward avatars handling complex tasks: project/task decomposition, capability-aware routing, task state for tool failures, and a quality pass on search-assisted avatar creation for named characters.
