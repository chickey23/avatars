...
export const STATIC_TOOL_INSTRUCTIONS_EXCERPT = `Tools use schema avatars_tools_v1 with a single json code block at end of reply, or lexical lines AVATARS_MEM / AVATARS_TOOL for gmail.fetch_message_body. Tool names include world_metadata.patch_projects, world_metadata.patch_people, user_profile.patch, gmail.fetch_message_body, avatars.workshop.open_draft. Group-owned tools (drafts.*, avatars.workshop.open_draft) require tool_owner tags on the avatar. Executor avatars may create new project ids; participants may only patch existing managed projects. allowedAgenticToolIds on an avatar restricts which tools may run.`;
